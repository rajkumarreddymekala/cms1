/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { LayoutSectionWhereUniqueInput } from "../../layoutSection/base/LayoutSectionWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class LayoutSectionUpdateManyWithoutLayoutsItemsInput {
  @Field(() => [LayoutSectionWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [LayoutSectionWhereUniqueInput],
  })
  connect?: Array<LayoutSectionWhereUniqueInput>;

  @Field(() => [LayoutSectionWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [LayoutSectionWhereUniqueInput],
  })
  disconnect?: Array<LayoutSectionWhereUniqueInput>;

  @Field(() => [LayoutSectionWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [LayoutSectionWhereUniqueInput],
  })
  set?: Array<LayoutSectionWhereUniqueInput>;
}

export { LayoutSectionUpdateManyWithoutLayoutsItemsInput as LayoutSectionUpdateManyWithoutLayoutsItemsInput };
