...
const CREATE_INPUT = {
  ...
  publishedVersion: 42,
  publishedSubversion: 42,
  currentVersion: 42,
  currentSubversion: 42,
  currentVersionStatus: "exampleCurrentVersionStatus",
  orderBy: 42,
...
const service = {
  ...
  layoutsItems: () => FIND_MANY_RESULT,
  layouts: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Layouts", () => {
  ...
  test("GET /layouts/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/layouts"}/${nonExistingId}`)
  ...
  test("GET /layouts/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/layouts"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /layouts existing resource", async () => {
